...
class DygestProcessor(DygestBaseParams):
    ...
    async def process_file(self, file: Path):
        ...
        if self.add_summaries and self.add_keywords:
            self.summaries, self.keywords = await self.create_summaries_and_keywords_async()
...
class SummaryProcessor:
    ...
    async def embed_summaries(self):
        ...
        for summary in self.summaries:
            ...
            response = await llms.get_aembeddings(text, model=self.embedding_model)
    ...
    async def embed_keywords(self):
        ...
        for keyword in self.keywords:
            response = await llms.get_aembeddings(keyword, model=self.embedding_model)
