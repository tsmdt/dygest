...
class WriterBase:
    ...
    expert_model: str = field(default=None, init=True)
    token_count: int = field(default=None, init=True)
    sentence_offsets: dict = field(default_factory=dict, init=True)  # NEW: Mapping from sentence ID (e.g. "S1") to char offset
    ...
    def add_main_content(self):
        ...
        for event in events:
            ...
            if event_type == 'start_entity':
                ...
                # Instead of closing spans and starting a new paragraph,
                # just insert the anchor inline at this position.

                # Ensure all spans are closed so the anchor doesn't appear nested incorrectly
                # If necessary, close spans, but do not change paragraphs:
                spans_stack.clear()
                current_parent = p_tag
                ...
                # Append inline anchor to the current parent
